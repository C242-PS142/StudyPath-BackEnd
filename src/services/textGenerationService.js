...
exports.generateText = async function (dateBirth, gender, data, callback) {
  const age = calculateAge(dateBirth);
  let genderText = "";
  if (gender === "L") {
    genderText = "Jenis Kelamin Laki-laki";
  } else if (gender === "P") {
    genderText = "Jenis Kelamin Perempuan";
  } else {
    genderText = ""; // Default value in case the gender is neither "L" nor "P"
  }
  let schoolLevel = "";
  if (age >= 13 && age <= 15) {
    schoolLevel = "SMP";
    ...
    schoolLevel = ""; // In case the age is outside the defined ranges
  }
  ...
  const Extroversion = Math.round(
    data["Keterbukaan Sosial, Energi, dan Antusiasme"] * 100,
  );
...
Saya Seorang Siswa ${schoolLevel}, ${genderText}, Usia ${age} Tahun, Saya ingin meminta rekomendasi untuk saya belajar yang sesuai dengan kepribadian saya, berikut data saya:
